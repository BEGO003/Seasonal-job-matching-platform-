...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!u) {
        // Try to fetch by id (may enrich fieldsOfInterest)
      ...
      // If missing interests, try enriching from API
      if (!Array.isArray((u as any).fieldsOfInterest)) {
        const fetched = await authApi.fetchCurrentUser();
        setUser(fetched || u);
        console.log("[Profile] Loaded user (enriched)", fetched || u);
      } else {
        setUser(u);
  ...
  const handleRemoveInterest = (index: number) => {
    const updated = {
      ...editedUser,
      fieldsOfInterest: editedUser.fieldsOfInterest.filter(
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (interestsChanged) {
        updates.fieldsOfInterest = editedUser.fieldsOfInterest || [];
      }
...
<SelectItem value="Zimbabwe">Zimbabwe</SelectItem>
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                {isEditing ? (
                  <div className="space-y-2">
                    <div className="flex flex-wrap gap-2">
                      {(editedUser?.fieldsOfInterest || []).map(
                        (f: string, i: number) => (
                          <Badge
                            key={i}
                            className="px-2 py-1 text-xs bg-primary/10 border-primary/20 text-primary flex items-center gap-1"
                          >
                            {f}
                            <button
                ...
                ) : Array.isArray(user.fieldsOfInterest) &&
                  user.fieldsOfInterest.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {user.fieldsOfInterest.map((f: string, i: number) => (
                      <Badge
                        key={i}
                  ...
                  <Input value="Not specified" readOnly />
                )}
              </div>
            </div>
          )}
